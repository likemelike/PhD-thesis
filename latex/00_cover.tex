...
\uestccover{面向城市计算应用的轨迹数据高性能算法设计}
            {计算机科学与技术}
            {202211081351}
            {李\hspace{1em}科}
            {商  烁}
...
% \uestccover[par]{面向城市计算应用的轨迹数据高性能算法设计}
% {计算机科学与技术}
% {202211081351}
% {李\hspace{1em}科}
% {商  烁}
...
\ClsLv{公开}  % \ClsLv{<密级>}
\UDC{004.65}  % \UDC{<UDC号>}
\DissertationTitle{面向城市计算应用的轨迹数据高性能算法设计}  % \DissertationTitle{<题名>}
...
\uestcentitlepage{High-Performance Algorithm Design for Trajectory Data in Urban Computing Applications}
                {Computer Science and Technology}
                {202211081351}
                {Li Ke}
                {Prof.Shang Shuo}{Prof.Chen Lisi}
...
\declaration[3cm]{}{authsign}[authsign]{spvrsign}